...
export default function EcrirePage() {
    async function handleFormSubmit(event: React.SyntheticEvent) {
        ...
        let mySection = urlParams.get('section');
        const createData = {
        ...
        try {
            const response = await axios.post('http://localhost:2900/chapter/ecrire', createData, {
                headers: {
            ...
            if (axios.isAxiosError(error)) {
                console.error('Failed to write chapter:', error.response?.data.message);
            } else {
